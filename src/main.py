...
from dummy_dataset import set_dummy_dataset, calculate # to set session state variables for dummy dataset
    ...
    for workspace_id, workspace_data in st.session_state.workspaces.items():
        ...
        pages['Your Workspaces'].append(
            st.Page(
                ...
                url_path=f'workspace_{workspace_id}', # because all workspaces reference workspace_page() callabler
...
def get_dummy_dataset() -> None:
    set_dummy_dataset(dummy_dataset='./res/dummy_dataset.csv') # use .csv dummy dataset
    calculate()
