...
def make_workspace_page(workspace_id: str) -> callable:
    def workspace_page(): # convert workspace_id to a callable that can be used as a page
        ...
        with st.container(border=True):
            ...
            with st.container(border=True):
                st.markdown('### Instructions & Exceptions:', unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            metric_summary_col_1.markdown('#### Pre-test Measurements:', unsafe_allow_html=True)
            ...
            metric_summary_col_2.markdown('#### Post-test Measurements:', unsafe_allow_html=True)
            ...
            st.markdown('### Effect Size Measurements:', unsafe_allow_html=True)
            try:
                st.table(
                    data={
                        'Metric': [
            ...
            with st.container(border=True):
                ...
                try:
                    ...
                    key_metrics_col_5.metric(label='Pre-test SD (s1)',  value=f'{pre_std:.2f}')
                    key_metrics_col_6.metric(label='Post-test SD (s2)', value=f'{post_std:.2f}')
                    key_metrics_col_7.metric(label='Pooled SD (sp)',    value=f'{pooled_std:.2f}')
        ...
        with st.container(border=True):
            st.markdown('# Pre-/Post-test Visualization', unsafe_allow_html=True)
            ...
            with st.container(border=True):
                st.markdown('### Pre- vs. Post- Box Plot Comparison:', unsafe_allow_html=True)
